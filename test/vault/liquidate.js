...
describe('Vault', () => {
  ...
  describe('#liquidate', () => {
    ...
    const subSetup = async () => {
      ...
      await vault.connect(trader).trade([expiry, strike, 1, toDecimalStr(-7), 0], now);
      await vault.connect(trader).trade([expiry, strike2, 1, toDecimalStr('-0.000000000000000001'), 0], now);
      await vault.connect(trader).trade([expiry, strike, 0, toDecimalStr(1), INT_MAX], now);
      return { vault, config, usdc, optionMarket };
    }
    ...
    context('when expired', () => {
      ...
      it('should revert with InvalidTime', async () => {
        await expectRevertCustom(vault.connect(liquidator).liquidate(trader.address, expiry, strike, true, toDecimalStr(7)), Vault, 'InvalidTime');
      });
    });
      ...
      context('when position size is not 0', () => {
        context('when healthFactor is 0.059418777692920392', () => {
          ...
          context('when liquidator has no balance', () => {
            it('should revert with InsufficientEquity', async () => {
              await expectRevertCustom(vault.connect(liquidator).liquidate(trader.address, expiry, strike, true, toDecimalStr(7)), Vault, 'InsufficientEquity');
            });
          });
        });
            ...
            context('when liquidate buy position', () => {
              ...
              context('when no sell position', () => {
                ...
                before(async () => {
                  ...
                  await vault.connect(trader).trade([expiry, strike, 0, toDecimalStr('0.000000000000000001'), INT_MAX], now);
                  await vault.connect(trader).trade([expiry, strike2, 1, toDecimalStr('6.000000000000000001'), INT_MAX], now);
            ...
            context('when liquidate sell position', () => {
              context('when size 0', () => {
                it('should revert with InvalidSize', async () => {
                  await expectRevertCustom(vault.connect(liquidator).liquidate(trader.address, expiry, strike, true, 0), Vault, 'InvalidSize');
                });
              });

              context('when size -1', () => {
                it('should revert with InvalidSize', async () => {
                  await expectRevertCustom(vault.connect(liquidator).liquidate(trader.address, expiry, strike, true, toDecimalStr(-1)), Vault, 'InvalidSize');
                });
              });
              ...
              context('when size 1', () => {
                context('when liquidator has no balance', () => {
                  it('should revert with Unavailable', async () => {
                    await expectRevertCustom(vault.connect(otherAccount).liquidate(trader.address, expiry, strike, true, toDecimalStr(1)), Vault, 'Unavailable');
                  });
                });
        ...
        context('when position size small', () => {
          ...
          before(async () => {
            ...
            await mintAndDeposit(vault, usdc, liquidator);
            await vault.connect(trader).trade([expiry, strike, 1, toDecimalStr(-0.7), 0], now);
            await spotPricer.setPrice(toDecimalStr(1200));
