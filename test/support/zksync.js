...
const wallets = accounts.map((a) => new Wallet(a.privateKey).connect(hre.ethers.provider));
const deployer = new Deployer(hre, wallets[0]);

async function getContractFactories(...contracts) {
  const contractFactories = [];
  for (let contract of contracts) {
    contractFactories.push(await zksyncGetContractFactory(contract));
  }
