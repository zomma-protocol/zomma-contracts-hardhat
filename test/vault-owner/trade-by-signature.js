...
describe('Vault', () => {
  ...
  let spotPricer, optionPricer, signatureValidator, vaultOwner, vaultProxy;
  ...
  describe('#tradeBySignature', () => {
    let vault, config, usdc, optionMarket, vaultOwner, vaultProxy, signedData;
          ...
          context('when hf < 1', () => {
            ...
            context('when then size 0.1', () => {
              ...
              before(async () => {
                ...
                await tradeBySignature(vaultProxy.connect(insuranceAccount), trader, [expiry, strike, 1, toDecimalStr(0.1), INT_MAX], now, gasFee);
            ...
            context('when then size 0.000000000000000001', () => {
              ...
              it('should revert with Unavailable', async () => {
                await expectRevertCustom(tradeBySignature(vaultProxy.connect(insuranceAccount), trader, [expiry, strike, 1, size, INT_MAX], now, gasFee), Vault, 'Unavailable');
              });
            });
          });
        });
